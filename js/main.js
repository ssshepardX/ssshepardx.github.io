document.addEventListener('DOMContentLoaded', function() {
    ...
    if (navLinksAnchors && navLinksAnchors.length > 0) {
    ...
    document.addEventListener('click', function(e) {
        if (menuToggle && navLinks && isMenuOpen) {
    ...
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape' && isMenuOpen && navLinks) {
            closeMenu();
        }
    });

    // Close menu on swipe (if user swipes from left to right on mobile)
    let touchStartX = 0;
    ...
    document.addEventListener('touchmove', function(e) {
        // Only prevent default if menu is open and it's a navigation element
        if (isMenuOpen && navLinks && navLinks.classList.contains('active')) {
            // Allow scrolling in menu
        }
    }, { passive: true });
    ...
    if (cvBtn && cvModal) {
        ...
        if (cvModalClose) {
            cvModalClose.addEventListener('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
                closeCVModal();
            });
        ...
        if (cvCloseBtn) {
            cvCloseBtn.addEventListener('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
                closeCVModal();
            });
    ...
    // ===== BUTTON OPTIMIZATION =====
    const allButtons = document.querySelectorAll('.btn, button, a[href*=".html"]');
    ...
    if (langSwitcher) {
        ...
        langBtns.forEach(btn => {
            btn.addEventListener('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
                // Language switching is handled by lang.js
            });
        });
    }
